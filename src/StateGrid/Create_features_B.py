...
@author: CharlesXu
@license: Q_S_Y_Q 
@file: Create_features_B.py
@time: 2018/1/24 14:38
@desc: 构建高敏感度的特征
"""
...
# --------------------------------------------load data---------------------------------------------------
print('@loading data...')
train_info = pd.read_csv(data_path_train + 'processed_' + file_jobinfo_train, sep='\t', encoding='utf-8',
...
test_info = pd.read_csv(data_path_test + 'processed_' + file_jobinfo_test, sep='\t', encoding='utf-8',
                        quoting=csv.QUOTE_NONE)
...
for i in train.topic.unique():
    ratio[i] = (len(train.loc[(train.topic == i) & (train.label == 1)]) + a) / (
            len(train.loc[train.topic == i]) + 2 * a)
